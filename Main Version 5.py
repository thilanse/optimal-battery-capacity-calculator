...
# get load data for a month of data and store it in a list
...
# return the inverter details by reading the csv
def inverter():
...
#get inverter price from the list based on the inverter size
def get_inverter_price(size):
...
def payback(size, savings, battery_cost):
    ...
    for i in range(len(savings)):
        ...
        details.append(
            [size[i], round(savings[i]), battery_cost[i], round(payback_in_days, 2), round(payback_in_years, 2)])
...
def inverter_propersize(inverter_size):
    ...
    inverter_sizes = [500, 1000, 1500, 2000, 2500, 3000, 3500, 4000, 4500, 5000, 5500, 6000, 6500, 7000, 7500, 8000,
...
def getNewEnergies(batt_dod, data, load, peak_energy, day_energy, off_energy):
    battery_load, battery_time = supplied_by_battery(data, batt_dod, load)
    # print(battery_time)
    batt_energy = energies(battery_load, battery_time)
    # print(batt_energy)
    ...
    return new_peak_energy, day_energy, new_off_energy
...
month_data = getMonthData('July')

batt, batt_dod, batt_size, batt_cost, weights = getBatteryList(dod)

max_load_for_the_month = inverter_size_for_month('July')
...
print(getDepreciatedCapacities(600,dep_rate, life_time))
...
def annual_saving_block(capacity):
    ...
    for row in month_data:
        ...
        new_peak_energy, new_day_energy, new_off_energy = getNewEnergies(batt_dod, data, load, peak_energy, day_energy, off_energy)
...
#calculate the annual savings based on a month of data and for a specific battery capacity
def annual_saving_tou(capacity):
    batt_dod = capacity*dod
...
def presentInflow(dep_caps):
    ...
    present_values = []
    for row in dep_caps:
        annual_savings = annual_saving_tou(row[1])
...
def presentOutflow(capacity):
    ...
    max_load = max_load_for_the_month

    inverter_size = (max_load * 230.0)
    proper_inverter_size = inverter_propersize(inverter_size)
    inverter_price = get_inverter_price(proper_inverter_size)
...
def NPV(capacity):
    ...
    presentOutflow_value = presentOutflow(capacity)
